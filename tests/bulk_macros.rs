...
fn keyword_macro_creates_types() {
    ...
    let _kw = Let::parse::<NoRules>(&mut input).unwrap();
...
fn punctuation_macro_parses() {
    ...
    let _p = Plus::parse::<NoRules>(&mut input).unwrap();
...
fn punctuation_macro_creates_enum() {
    ...
    let p = Punctuation::parse::<NoRules>(&mut input).unwrap();
...
fn literals_macro_captures() {
    ...
    let lit = IntLit::parse::<NoRules>(&mut input).unwrap();
    assert_eq!(lit.0, "42");
}
...
fn literals_macro_creates_enum() {
    ...
    let lit = Literal::parse::<NoRules>(&mut input).unwrap();
